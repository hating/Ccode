...
Tree_Node *ID3(NodeSample *head)
{
	...
	if(strcmp(tree->root,"root"))
	{
		...
		{
			new_head=pick_items(tree->root,branchs[loop],head);
			tree_temp=ID3(new_head);
			for(i_third=0;i_third<4;i_third++)
			{
...
void inputDrillSample(void)
{
	...
	for(i=0;i<number;i++)
	{
		...
		printf("Weather(Sunny\\Overcast\\Rain)£º");
		scanf("%s",sample[i].sample.item[0]);
		printf("Temperature(Hot\\Mild\\Cool)£º");
		scanf("%s",sample[i].sample.item[1]);
		printf("Humidity(High\\Normal)£º");
		scanf("%s",sample[i].sample.item[2]);
		printf("Windy(Strong\\Weak)£º");
		scanf("%s",sample[i].sample.item[3]);
		printf("Play tennis or not(Yes\\No)£º");
