...
My_Data pick_items(const string &att,const string &val,const My_Data &data)//筛选出符合条件的数据组成新的数据 
{
    ...
    else
    {
        ...
        for(vector< vector<string> >::const_iterator iter=data.items.begin();iter!=data.items.end();++iter)//遍历旧data 
            if((*iter)[num]==val)//如果与val(Sunny,Wind,Overcast)相等 
...
Decision_Tree ID3(const My_Data &data)
{
    ...
    if(tree.get_root()!="ROOT")//如果root被修改，找出合适的数据，并且调用ID3算法 
    {
        vector<string> branchs=att_val(tree.get_root(),data);//选出该属性所拥有的分支 
        for(vector<string>::const_iterator iter=branchs.begin();iter!=branchs.end();++iter)//每个分支都检验一遍，iter=sunny or wind  
        {
            My_Data new_data=pick_items(tree.get_root(),*iter,data);//拿出符合iter的数据 
            tree_temp=ID3(new_data);//递归调用ID3 
            tree.set_branch(*iter,tree_temp);//把tree_temp挂在tree树的下面 
        }
    }
    else
    {
        tree.set_leaf(true);//设置叶子节点 
        tree.set_root((data.items[0])[data.col-1]);//把root设置成Yes或者No 
    }
    return tree;
}
